...
async function applyMigration() {
  ...
  try {
    console.log("🔄 Applying billing period to plans migration...");
    ...
    if (Array.isArray(tableInfo) && tableInfo.length > 0) {
      console.log("✅ Column 'billingPeriod' already exists. Migration already applied.");
      return;
    }
    ...
    // Then add billingPeriod column
    await prisma.$executeRaw`
      ALTER TABLE "plans" 
      ADD COLUMN IF NOT EXISTS "billingPeriod" "PlanBillingPeriod" NOT NULL DEFAULT 'MONTHLY';
    `;

    console.log("✅ Migration applied successfully!");
    console.log("   - Added 'billingPeriod' column (PlanBillingPeriod, default: MONTHLY) to 'plans' table");
  } catch (error) {
