...
async function applyMigration() {
  ...
  try {
    console.log("🔄 Applying email verification attempts migration...");
    ...
    const tableInfo = await prisma.$queryRaw`
      ...
      AND column_name IN ('attemptCount', 'lastAttemptAt')
    `;
    ...
    if (Array.isArray(tableInfo) && tableInfo.length > 0) {
      ...
      if (
        existingColumns.includes("attemptCount") &&
        existingColumns.includes("lastAttemptAt")
      ) {
    ...
    console.log("   - Added 'attemptCount' column (INTEGER, default: 0)");
    console.log("   - Added 'lastAttemptAt' column (TIMESTAMP(3), nullable)");
  } catch (error) {
